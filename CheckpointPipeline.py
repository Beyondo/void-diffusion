...
def convert_ldm_unet_checkpoint(checkpoint, config, path=None, extract_ema=False):
    ...
    if sum(k.startswith("model_ema") for k in keys) > 100:
        print(f"VOID Checkpoint Loader: Checkpoint {path} has both EMA and non-EMA weights.")
        if extract_ema:
            print("VOID Checkpoint Loader: Extracting EMA weights (usually better for inference)")
            for key in keys:
            ...
            print(
                "VOID Checkpoint Loader: Extracting only the non-EMA weights (usually better for fine-tuning)"
            )
...
def load_pipeline_from_original_stable_diffusion_ckpt(
    ...
    if vae_path:
        print(f"VOID Checkpoint Loader: Converting VAE {vae_path}")
    ...
    # an externally loaded diffusers VAE was not passed
        ...
        print("VOID Checkpoint Loader: Using checkpoint model's original VAE")

    if vae:
        print("VOID Checkpoint Loader: Using replacement diffusers VAE")
