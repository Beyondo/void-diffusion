...
def init(ModelName):
    ...
    if not torch.cuda.is_available():
        ...
        try:
            ...
            # CLIPTextConfig
            config = CLIPTextConfig.from_pretrained("openai/clip-vit-base-patch32", revision=rev, torch_dtype=torch.float16)
            config.max_position_embeddings = 512
            tokenizer = CLIPTokenizer.from_pretrained("openai/clip-vit-base-patch32", revision=rev, torch_dtype=torch.float16)
